...
@section('title', 'Chat dengan ' . ($session->vendor?->name ?? 'Vendor'))
@section('page-title', 'Chat Vendor')

@section('content')
<div class="mb-4 flex items-center gap-3">
    <a href="{{ route('chat.internal.admin') }}"
    ...
    <span class="text-sm font-semibold text-dark">{{ $session->vendor?->name ?? 'Vendor' }}</span>
...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col" style="height: calc(100vh - 150px); min-height: 480px;">
    ...
    {{-- Messages --}}
    <div id="internal-admin-messages"
        ...
        <div class="flex flex-col items-center justify-center h-full text-gray-400 py-10">
            ...
            <p class="text-xs text-gray-400 mt-1">Vendor belum mengirim pesan.</p>
        </div>
        @endif

        @foreach($session->messages as $msg)
        @php $isMine = $msg->sender === 'admin'; @endphp
    ...
    <div id="internal-admin-input-area" class="border-t border-gray-100 px-4 pt-2 pb-3">
        <div class="flex items-end gap-2">
            ...
            <button type="button" id="internal-admin-reply-btn"
...
(function () {
    ...
    var vendorName = '{{ addslashes($session->vendor?->name ?? "Vendor") }}';
    ...
    function appendMessage(msg) {
        ...
        var isMine = msg.sender === 'admin';
        var wrap = document.createElement('div');
        wrap.className = 'flex ' + (isMine ? 'justify-end' : 'justify-start');
        wrap.dataset.msgId = msg.id;
        var senderLabel = !isMine
            ? '<span class="block text-[10px] mb-1 font-medium text-gray-400">' + esc(vendorName) + '</span>'
            : '';
    ...
    if (!sessionClosed) {
        setInterval(function () {
            fetch('/dashboard/chat-internal/' + token + '/poll?after=' + lastId, {
    ...
    function sendReply() {
        ...
        fetch('/dashboard/chat-internal/' + token + '/reply', {
            method: 'POST',
            headers: {
